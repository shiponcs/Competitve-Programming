...
int g, cap;
bool check = false;
int val[10010], wgt[10010], dp[35][1009];
...
int knapsack()
{
	for(int i = 1; i <=  cap; i++){
		for(int j = 1; j <= n; j++){
			...
			if(i < wgt[j - 1]) dp[i][j] = dp[i][j - 1];
			else
				dp[i][j] = max(dp[i][j - 1], val[j - 1] + dp[i - wgt[j - 1]][j - 1]);
...
int main()
{
	...
	while(t--){
		...
		cin >> g;
		ans = 0;
		FOR(i, 0, g) {memset(dp, 0, sizeof dp);cin >> cap; ans += knapsack();}
